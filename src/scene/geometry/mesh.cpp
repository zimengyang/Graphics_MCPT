...
Intersection Mesh::SampleOnGeometrySurface(const float &u, const float &v, const glm::vec3 &point)
{
    ...
    sample.normal = glm::normalize(glm::vec3(transform.invTransT() * glm::vec4(sample.normal,0.0f)));
    sample.tangent = glm::normalize(glm::vec3(transform.invTransT() * glm::vec4(sample.tangent,0.0f)));
    sample.bitangent = glm::normalize(glm::vec3(transform.invTransT() * glm::vec4(sample.bitangent,0.0f)));
    return sample;
}
...
Intersection Mesh::GetIntersection(Ray r){
    ...
    if(closest.object_hit != NULL)
    {
        ...
        //Store the tangent and bitangent
        closest.tangent = glm::normalize(glm::vec3(transform.invTransT() * glm::vec4(closest.tangent, 0.0f)));
        closest.bitangent = glm::normalize(glm::vec3(transform.invTransT() * glm::vec4(closest.bitangent, 0.0f)));
