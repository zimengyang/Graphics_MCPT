...
glm::vec4 GetCubeTangent(const glm::vec4& P)
{
    glm::vec3 T;
    glm::vec3 up(0,1,0);
    ...
    T = glm::cross(up, glm::vec3(GetCubeNormal(P)));
    return glm::vec4(T, 0);
}

glm::vec4 GetCubeBitangent(const glm::vec4& P)
{
    glm::vec3 B = glm::cross(glm::vec3(GetCubeNormal(P)),
                             glm::vec3(GetCubeTangent(P)));
    return glm::vec4(B, 0);
}
...
Intersection Cube::SampleOnGeometrySurface(const float &u, const float &v, const glm::vec3 &point)
{
    ...
    if(inter.t > 0)
    {
        ...
        Intersection sample;
        sample.point = glm::vec3(transform.T() * glm::vec4(samplePoint,1.0f));
        sample.t = 1.0f;
        sample.normal = glm::normalize(glm::vec3(transform.invTransT() * GetCubeNormal(glm::vec4(samplePoint,1.0f))));
        sample.object_hit = this;
        sample.tangent = glm::normalize(glm::vec3(transform.invTransT() * GetCubeTangent(glm::vec4(samplePoint,1.0f))));
        sample.bitangent = glm::normalize(glm::vec3(transform.invTransT() * GetCubeBitangent(glm::vec4(samplePoint,1.0f))));
...
Intersection Cube::GetIntersection(Ray r)
{
    ...
    if(t_final >= 0)
    {
        ...
        result.tangent = glm::normalize(glm::vec3(transform.invTransT() * GetCubeTangent(P)));
        result.bitangent = glm::normalize(glm::vec3(transform.invTransT() * GetCubeBitangent(P)));
